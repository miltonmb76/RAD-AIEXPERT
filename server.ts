...
import {
  normalizeProtocolDecisionGraph,
  buildFallbackGraphFromScorecard,
} from "./src/lib/scorecardReasoningBridge";
...
/**
 * API: PUENTE SCORECARD → GRAFO DE DECISIÓN DEL PROTOCOLO
 * POST /api/generate-scorecard-decision-graph
 */
app.post("/api/generate-scorecard-decision-graph", async (req: express.Request, res: express.Response) => {
  try {
    const { model, report, studyType, scorecard, protocolId } = req.body;
    const hasScorecard = scorecard && Array.isArray(scorecard.criteria) && scorecard.criteria.length > 0;
    if (!hasScorecard && !(report && String(report).trim())) {
      return res.status(400).json({
        success: false,
        error: "Se requiere un Scorecard o el texto del informe.",
      });
    }

    const ai = getGeminiClient();
    const modelToUse = getModelName(model);
    const sc = hasScorecard ? scorecard : null;
    const criteriaBlock = sc
      ? sc.criteria
          .map(
            (c: any, i: number) =>
              `${i + 1}. id=${c.id} | ${c.criterion} | status=${c.status} | value=${c.value || ""} | weight=${c.weight || ""} | evidence=${c.evidence || ""}`
          )
          .join("\n")
      : "(Sin scorecard previo: inferí criterios del protocolo a partir del informe y enlazalos en linkedCriterionId.)";

    const prompt = `Eres un radiólogo hispanohablante experto en protocolos diagnósticos.
Construye el GRAFO DE DECISIÓN DEL PROTOCOLO (puente Scorecard → razonamiento): nodos ordenados que representan cómo se recorre el algoritmo clínico, iluminados por el estado real de cada criterio.

IDIOMA: español médico en labels/detalles/narrativa.

ESTUDIO: ${studyType || "No especificado"}
PROTOCOLO: ${sc?.protocolId || protocolId || "auto"} — ${sc?.protocolName || "detectar"}
CATEGORIA ASIGNADA: ${sc?.categoryAssigned || "a determinar"}
SCORE: ${sc ? `${sc.scoreMet}/${sc.scoreTotal}` : "n/d"} | SEMÁFORO: ${sc?.trafficLight || "n/d"}

CRITERIOS DEL SCORECARD (fuente de verdad):
${criteriaBlock}

${report ? `INFORME:\n"""\n${report}\n"""` : "Sin informe adicional (usa solo el scorecard)."}

TAREA:
1. Crea nodes en orden de decisión del protocolo:
   - 1 nodo start
   - 5 a 10 nodos criterion (uno por criterio relevante; linkedCriterionId = id del scorecard)
   - 0-2 nodos gate si hay bifurcaciones clave
   - 1 nodo category (resultado / categoría)
   - 0-1 nodo action (siguiente paso diagnóstico, NO terapéutica invasiva inventada)
2. status de criterion DEBE reflejar el status del scorecard enlazado (met|not_met|not_mentioned|equivocal).
3. edges: from/to conectando el recorrido; active=true en aristas del camino recorrido (criterios met/equivocal hacia el outcome).
4. pathNarrative: 2-4 frases explicando cómo el caso caminó el protocolo y qué criterios faltaron.
5. outcomeLabel = categoría/resultado final.

Claves JSON obligatorias:
title, protocolId, protocolName, categoryAssigned, trafficLight, scoreMet, scoreTotal, nodes, edges, pathNarrative, outcomeLabel, clinicalSummary.
Cada node: id, kind (start|criterion|gate|category|action), label, linkedCriterionId?, status, value?, evidence?, weight?, detail?.
Cada edge: id, from, to, label?, active?
`;
    ...
    const fullSchema = {
      ...
      properties: {
        ...
        pathNarrative: { type: Type.STRING },
    ...
    try {
      ...
      console.warn(
        "generate-scorecard-decision-graph: schema failed:",
        schemaErr?.message || schemaErr
      );
    }

    if (!parsed || !(parsed.nodes || parsed.nodos)?.length) {
      try {
        const response2 = await ai.models.generateContent({
          model: modelToUse,
          contents: prompt + `\n\nSOLO JSON válido. nodes obligatorio.`,
          config: { temperature: 0.2, responseMimeType: "application/json" },
        });
        rawText = readModelText(response2) || rawText;
        parsed = extractJsonObject(rawText) || parsed;
      } catch (e: any) {
        console.warn("generate-scorecard-decision-graph: mime retry failed:", e?.message || e);
      }
    }

    let data = normalizeProtocolDecisionGraph(parsed || {}, sc);
    if (!data.nodes.length && sc) {
      data = buildFallbackGraphFromScorecard(sc);
    }
    if (!data.nodes.length) {
      return res.status(500).json({
        success: false,
        error: "No se pudo construir el grafo de decisión del protocolo.",
      });
    }

    res.json({ success: true, data, scorecard: sc || undefined });
  } catch (error: any) {
    console.error("Error en /api/generate-scorecard-decision-graph:", error);
    res.status(500).json({ success: false, error: handleGeminiError(error) });
  }
});
