...
import { AbdominalWall3DData, AbdominalWall3DPanel, AbdominalWallFindingRow } from "../types";
...
/**
 * Renders a two-page "ANEXO: SUITE PARED ABDOMINAL 3D & FICHA DE PARED" into the provided jsPDF document.
 */
export async function renderAbdominalWall3DPageToPdf(
  doc: jsPDF,
  abdominalWallData: AbdominalWall3DData,
  pageSize: "letter" | "a4" = "letter",
  pdfLayoutType: string = "modern"
): Promise<void> {
  if (!abdominalWallData || (!abdominalWallData.panels?.length && !abdominalWallData.findingTable?.length)) {
    return;
  }
  ...
  doc.text("ANEXO: SUITE PARED ABDOMINAL 3D & FICHA DE PARED", marginX, yCoord);
  yCoord += 4.5 * factor;
  ...
  const territory = abdominalWallData.territoryLabel || "ECOGRAFÍA DE PARED ABDOMINAL";
  const figTitle = abdominalWallData.figureTitle || `FIGURA 1. ATLAS 3D PARED ABDOMINAL Y CORRELACIÓN DE DEFECTOS — ${territory.toUpperCase()}`;
  ...
  const panels: AbdominalWall3DPanel[] = (abdominalWallData.panels || []).filter(p => p && (p.imageUrl || p.panelTitle));
  ...
  if (panelCount > 0) {
    ...
    const measureCaptionH = (p: AbdominalWall3DPanel): number => {
      doc.setFont("helvetica", "bold");
    ...
    for (let idx = 0; idx < panelCount; idx++) {
      ...
      if (p.imageUrl && p.imageUrl.startsWith("data:image")) {
        try {
          ...
          console.warn("Error drawing abdominal wall 3D image to PDF:", imgErr);
        ...
        doc.text("Reconstrucción 3D Pared", imgX + (imgWidth / 2) - 18, imgY + (imgHeight / 2));
      }
  ...
  const dossierBlocks: Array<{ title: string; text: string; color: [number, number, number] }> = [
    { title: "RESUMEN DE PARED", text: pdfCutawayToCorte(String(abdominalWallData.wallSummary || "").trim()), color: accent },
    { title: "MORFOLOGÍA DEL DEFECTO", text: pdfCutawayToCorte(String(abdominalWallData.morphologyNotes || "").trim()), color: [3, 105, 161] },
    { title: "CAPAS / FASCIA", text: pdfCutawayToCorte(String(abdominalWallData.wallLayersStatus || "").trim()), color: [12, 74, 110] },
  ];
  const keyPoints = Array.isArray(abdominalWallData.keyPoints) ? abdominalWallData.keyPoints.filter(Boolean) : [];
  if (keyPoints.length) {
    dossierBlocks.push({
      title: "PUNTOS CLAVE",
  ...
  if (dossierTexts.length) {
    ...
    doc.setTextColor(15, 23, 42);
    doc.text("FICHA CLÍNICA PARED ABDOMINAL (CORRELACIÓN CON LA FIGURA 3D)", marginX, dossierY);
    dossierY += 3.4 * factor;
  ...
  doc.setTextColor(15, 23, 42);
  doc.text("ANEXO: SUITE PARED ABDOMINAL 3D — TABLA Y SÍNTESIS", marginX, yCoord);
  yCoord += 4.2 * factor;
  doc.setDrawColor(accent[0], accent[1], accent[2]);
  ...
  const tableData: AbdominalWallFindingRow[] = abdominalWallData.findingTable || [];
  const tableTitle = abdominalWallData.tableTitle || "TABLA ECOGRÁFICA DE PARED ABDOMINAL:";
  ...
  const synthText = abdominalWallData.morphologicalSynthesis || "";
  if (synthText && synthText.trim()) {
    const footerSafeBottom = pageHeight - 18 * factor;
    const synthTitle = abdominalWallData.synthesisTitle || "SÍNTESIS MORFOLÓGICA DE PARED ABDOMINAL:";
    doc.setFont("helvetica", "normal");
    doc.setFontSize(9.0 * factor);
    ...
    const startSynthContinuationPage = () => {
      ...
      doc.text("Continuación — síntesis morfológica de pared abdominal", marginX, yCoord);
      yCoord += 6 * factor;
    };
