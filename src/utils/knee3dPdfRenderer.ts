...
/**
 * Renders an exclusive, full-page "ANEXO: SUITE RODILLA 3D & FICHA LIGAMENTOS Y MENISCOS" into the provided jsPDF document.
 */
export async function renderKnee3DPageToPdf(
  ...
  const dossierBlocks: Array<{ title: string; text: string; color: [number, number, number] }> = [
    { title: "RESUMEN DEL RODILLA", text: String(kneeData.kneeSummary || "").trim(), color: accent },
    { title: "MORFOLOGÍA / TENDÓN DOMINANTE", text: String(kneeData.morphologyNotes || "").trim(), color: [3, 105, 161] },
    { title: "ESTADO DEL RODILLA", text: String(kneeData.ligamentMeniscusStatus || "").trim(), color: [12, 74, 110] },
  ];
  ...
  if (dossierTexts.length) {
    ...
    doc.setTextColor(15, 23, 42);
    doc.text("FICHA CLÍNICA RODILLA / RODILLA (CORRELACIÓN CON LA FIGURA 3D)", marginX, dossierY);
    dossierY += 3.2 * factor;
    rowY = dossierY;
  ...
  if (synthText && synthText.trim()) {
    ...
    const synthTitle = kneeData.synthesisTitle || "SÍNTESIS MORFOLÓGICA Y FUNCIONAL DEL RODILLA:";
    doc.setFont("helvetica", "normal");
