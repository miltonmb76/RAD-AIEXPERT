 ...
 */
export async function renderScrotum3DPageToPdf(
  doc: jsPDF,
  scrotumData: Scrotum3DData,
  pageSize: "letter" | "a4" = "letter",
  pdfLayoutType: string = "modern"
): Promise<void> {
  if (!scrotumData || (!scrotumData.panels?.length && !scrotumData.findingTable?.length)) {
    return;
  }
  ...
  doc.text("ANEXO: SUITE ESCROTO 3D & FICHA ESCROTAL", marginX, yCoord);
  yCoord += 4.5 * factor;
  ...
  const territory = scrotumData.territoryLabel || "ECOGRAFÍA DE ESCROTO";
  const figTitle = scrotumData.figureTitle || `FIGURA 1. ATLAS 3D ESCROTO Y CORRELACIÓN ANATOMOPATOLÓGICA — ${territory.toUpperCase()}`;
  ...
  const panels: Scrotum3DPanel[] = (scrotumData.panels || []).filter(p => p && (p.imageUrl || p.panelTitle));
    ...
    for (let idx = 0; idx < panelCount; idx++) {
      ...
      if (p.imageUrl && p.imageUrl.startsWith("data:image")) {
        try {
          ...
          console.warn("Error drawing scrotum 3D image to PDF:", imgErr);
  ...
  if (dossierTexts.length) {
    ...
    doc.setTextColor(15, 23, 42);
    doc.text("FICHA CLÍNICA ESCROTAL (CORRELACIÓN CON LA FIGURA 3D)", marginX, dossierY);
    dossierY += 3.4 * factor;
  ...
  doc.setTextColor(15, 23, 42);
  doc.text("ANEXO: SUITE ESCROTO 3D — TABLA Y SÍNTESIS", marginX, yCoord);
  yCoord += 4.2 * factor;
  doc.setDrawColor(accent[0], accent[1], accent[2]);
  ...
  const tableData: ScrotumFindingRow[] = scrotumData.findingTable || [];
  const tableTitle = scrotumData.tableTitle || "TABLA ECOGRÁFICA DE ESCROTO:";
  ...
  const synthText = scrotumData.morphologicalSynthesis || "";
  if (synthText && synthText.trim()) {
    const footerSafeBottom = pageHeight - 18 * factor;
    const synthTitle = scrotumData.synthesisTitle || "SÍNTESIS MORFOLÓGICA ESCROTAL:";
    doc.setFont("helvetica", "normal");
    doc.setFontSize(9.0 * factor);
    ...
    const startSynthContinuationPage = () => {
      ...
      doc.text("Continuación — síntesis morfológica escrotal", marginX, yCoord);
      yCoord += 6 * factor;
    };
