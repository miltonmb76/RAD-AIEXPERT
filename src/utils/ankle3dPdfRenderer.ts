...
import { Ankle3DData, Ankle3DPanel, AnkleFindingRow } from "../types";
 ...
 */
export async function renderAnkle3DPageToPdf(
  doc: jsPDF,
  ankleData: Ankle3DData,
  pageSize: "letter" | "a4" = "letter",
  pdfLayoutType: string = "modern"
): Promise<void> {
  if (!ankleData || (!ankleData.panels?.length && !ankleData.findingTable?.length)) {
    return;
  }
  ...
  doc.text("ANEXO: SUITE TOBILLO 3D & FICHA LIGAMENTOS Y AQUILES", marginX, yCoord);
  yCoord += 4.5 * factor;
  ...
  const territory = ankleData.territoryLabel || "ECOGRAFÍA DE TOBILLO";
  const figTitle = ankleData.figureTitle || `FIGURA 1. ATLAS 3D TOBILLO Y CORRELACIÓN TOBILLO — ${territory.toUpperCase()}`;
  ...
  doc.setFillColor(247, 254, 231); // sky-50
  doc.setDrawColor(217, 249, 157); // sky-200
  ...
  const panels: Ankle3DPanel[] = (ankleData.panels || []).filter(p => p && (p.imageUrl || p.panelTitle));
  ...
  if (panelCount > 0) {
    ...
    const measureCaptionH = (p: Ankle3DPanel): number => {
      doc.setFont("helvetica", "bold");
    ...
    for (let idx = 0; idx < panelCount; idx++) {
      ...
      if (p.imageUrl && p.imageUrl.startsWith("data:image")) {
        try {
          ...
          console.warn("Error drawing ankle 3D image to PDF:", imgErr);
  ...
  const dossierBlocks: Array<{ title: string; text: string; color: [number, number, number] }> = [
    { title: "RESUMEN DEL TOBILLO", text: pdfCutawayToCorte(String(ankleData.ankleSummary || "").trim()), color: accent },
    { title: "MORFOLOGÍA / NOTAS ESTRUCTURALES", text: pdfCutawayToCorte(String(ankleData.morphologyNotes || "").trim()), color: [77, 124, 15] },
    { title: "ESTADO LIGAMENTOS / AQUILES", text: pdfCutawayToCorte(String(ankleData.ligamentAchillesStatus || "").trim()), color: [54, 83, 20] },
  ];
  const keyPoints = Array.isArray(ankleData.keyPoints) ? ankleData.keyPoints.filter(Boolean) : [];
  if (keyPoints.length) {
    dossierBlocks.push({
      title: "PUNTOS CLAVE",
  ...
  if (dossierTexts.length) {
    ...
    doc.setTextColor(15, 23, 42);
    doc.text("FICHA CLÍNICA TOBILLO / LIGAMENTOS-AQUILES (CORRELACIÓN CON LA FIGURA 3D)", marginX, dossierY);
    dossierY += 3.4 * factor;
  ...
  doc.setTextColor(15, 23, 42);
  doc.text("ANEXO: SUITE TOBILLO 3D — TABLA Y SÍNTESIS", marginX, yCoord);
  yCoord += 4.2 * factor;
  doc.setDrawColor(accent[0], accent[1], accent[2]);
  ...
  const tableData: AnkleFindingRow[] = ankleData.findingTable || [];
  const tableTitle = ankleData.tableTitle || "TABLA ECOGRÁFICA DE LIGAMENTOS, AQUILES Y ESTRUCTURAS PERIARTICULARES:";
  ...
  doc.setFillColor(236, 252, 203); // cyan-50
  ...
  const synthText = ankleData.morphologicalSynthesis || "";
  if (synthText && synthText.trim()) {
    const footerSafeBottom = pageHeight - 18 * factor;
    const synthTitle = ankleData.synthesisTitle || "SÍNTESIS MORFOLÓGICA Y FUNCIONAL DE TOBILLO:";
    doc.setFont("helvetica", "normal");
    doc.setFontSize(9.0 * factor);
    ...
    const startSynthContinuationPage = () => {
      ...
      doc.text("Continuación — síntesis morfológica y funcional de ligamentos/Aquiles", marginX, yCoord);
      yCoord += 6 * factor;
    };
