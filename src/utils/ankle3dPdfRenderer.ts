...
export async function renderAnkle3DPageToPdf(
  ...
  const dossierBlocks: Array<{ title: string; text: string; color: [number, number, number] }> = [
    { title: "RESUMEN DEL TOBILLO", text: String(ankleData.ankleSummary || "").trim(), color: accent },
    { title: "MORFOLOGÍA / NOTAS ESTRUCTURALES", text: String(ankleData.morphologyNotes || "").trim(), color: [77, 124, 15] },
    { title: "ESTADO LIGAMENTOS / AQUILES", text: String(ankleData.ligamentAchillesStatus || "").trim(), color: [54, 83, 20] },
  ];
