...
import {
  AbdominalWall3DData,
  AbdominalWall3DPanel,
  AbdominalWallFindingRow,
  AbdominalWallStudyType,
  ClinicalScorecardData
} from "../types";
import { runBackgroundTask } from "../lib/backgroundTasks";
import { buildAbdominalWallDirectivesFromScorecard, ABDOMINAL_WALL_TOPOGRAPHY_DIRECTIVE } from "../lib/clinicalIntelligence";
...
export const AbdominalWall3DModule: React.FC<AbdominalWall3DModuleProps> = ({
  ...
  const [zoomPanel, setZoomPanel] = useState<AbdominalWall3DPanel | null>(null);
    ...
    if (externalDirectives && externalDirectives.trim()) {
      setCustomDirectives((prev) => {
        if (
          ...
          prev.includes("DIRECTIVA OBLIGATORIA DEL SCORECARD DE PARED ABDOMINAL")
        ) {
  ...
  React.useEffect(() => {
    const fromScorecard = buildAbdominalWallDirectivesFromScorecard(scorecardData || null);
  ...
  const mergeMandatoryDirectives = (extraPanelDirective?: string) => {
    const scorecardDirectives = buildAbdominalWallDirectivesFromScorecard(scorecardData || null);
    const userExtra = customDirectives.trim();
    const extraIsScorecardEcho =
      !!scorecardDirectives &&
      !!userExtra &&
      (userExtra === scorecardDirectives ||
        userExtra.includes("DIRECTIVA OBLIGATORIA DEL SCORECARD DE PARED ABDOMINAL") ||
  ...
  const [selectedAbdominalWallType, setSelectedAbdominalWallType] = useState<AbdominalWallStudyType>(() => {
    const text = ((reportText || "") + " " + (activeProtocol || "")).toLowerCase();
    if (text.includes("inguinal") || text.includes("crural") || text.includes("femoral") || text.includes("ingle")) return "hernia_inguinal_crural";
    if (text.includes("umbilical") || text.includes("epigastr") || text.includes("linea alba") || text.includes("línea alba")) return "hernia_umbilical_epigastrica";
    if (text.includes("diastasis") || text.includes("diástasis") || text.includes("eventracion") || text.includes("eventración") || text.includes("recto abdominal")) return "diastasis_recto_eventracion";
    if (text.includes("pared abdominal") || text.includes("hernia") || text.includes("valsalva")) return "general_pared_abdominal";
    return "general_pared_abdominal";
  });
  ...
  const handleGenerate = async () => {
    if (!reportText || !reportText.trim()) {
      setErrorMessage("No hay informe disponible para procesar la suite de pared abdominal.");
      return;
    }

    setIsGenerating(true);
    setErrorMessage(null);
    setGenerationStep("Analizando pared abdominal, defectos herniarios y capas fasciales...");

    try {
      setGenerationStep("Construyendo paneles 3D y ficha de pared abdominal...");

      const mergedDirectives = mergeMandatoryDirectives();

      await runBackgroundTask("abdominal-wall-3d", "Generando Suite Pared Abdominal 3D", async () => {
        const response = await fetch("/api/generate-3d-abdominal-wall", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            reportText,
            abdominalWallType: selectedAbdominalWallType,
        ...
        if (!resData.success) {
          throw new Error(resData.error || "Error al generar la Suite Pared Abdominal 3D.");
        }

        setAbdominalWallData(resData.data);
        setIncludeInReport(true);
      });
      setGenerationStep("");
    } catch (err: any) {
      console.error("Error generando Suite Pared Abdominal 3D:", err);
      setErrorMessage(err.message || "Error al procesar la Suite Pared Abdominal.");
    } finally {
  ...
  const handleFlipHorizontal = async (panelLetter: string) => {
    if (!abdominalWallData) return;
    const panel = abdominalWallData.panels.find((p) => p.panelLetter === panelLetter);
    if (!panel?.imageUrl) return;
    try {
      const flippedDataUrl = await flipImageDataUrl(panel.imageUrl);
      const updatedPanels = abdominalWallData.panels.map((p) => {
      ...
      console.error("Error al voltear panel de pared abdominal:", flipErr);
  ...
  const handleDeleteSinglePanel = (panelLetter: string) => {
    if (!abdominalWallData || abdominalWallData.panels.length <= 1) return;

    const remainingPanels = abdominalWallData.panels.filter((p) => p.panelLetter !== panelLetter);
    ...
    let updatedTitle = abdominalWallData.figureTitle;
    if (updatedTitle) {
  ...
  const handleRegenerateSinglePanel = async (panel: AbdominalWall3DPanel) => {
    if (!abdominalWallData) return;
    ...
    try {
      const response = await fetch("/api/regenerate-3d-abdominal-wall-panel", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          reportText,
      ...
      console.error("Error regenerando panel de pared abdominal:", err);
  ...
  const handleAddTableRow = () => {
    if (!abdominalWallData) return;
    const newRow: AbdominalWallFindingRow = {
      location: "Umbilical",
      structure: "Fascia / orificio",
      sizeOrGap: "—",
      wallLayers: "Capas conservadas",
      content: "Vacío / grasa",
      reducibilityOrDynamic: "Reducible / Valsalva",
      severity: "Leve",
      clinicalImpact: "Seguimiento clínico"
    };
    setAbdominalWallData({
      ...abdominalWallData,
      findingTable: [...(abdominalWallData.findingTable || []), newRow]
    });
  };

  const handleDeleteTableRow = (idx: number) => {
    if (!abdominalWallData) return;
    const updated = [...(abdominalWallData.findingTable || [])];
  ...
  const handleUpdateTableRow = (idx: number, field: keyof AbdominalWallFindingRow, value: string) => {
    if (!abdominalWallData) return;
    const updated = [...(abdominalWallData.findingTable || [])];
    ...
    <div className="bg-white rounded-xl shadow-lg border border-amber-100 overflow-hidden transition-all duration-300">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="font-bold text-base md:text-lg tracking-wide text-white">
                Suite Pared Abdominal 3D & Ficha de Pared
              </h3>
            ...
            <p className="text-xs text-amber-100/80">
              Reconstrucción 3D de hernias, diástasis, capas fasciales y dinámica con Valsalva
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
          <div>
            <label className="block text-xs font-bold text-slate-700 mb-1 flex items-center gap-1">
              ...
              Tipo de estudio de pared abdominal
            </label>
            <select
              value={selectedAbdominalWallType}
              onChange={(e) => setSelectedAbdominalWallType(e.target.value as AbdominalWallStudyType)}
            ...
            >
              <option value="hernia_inguinal_crural">Hernia inguinal / crural</option>
              <option value="hernia_umbilical_epigastrica">Hernia umbilical / epigástrica</option>
              <option value="diastasis_recto_eventracion">Diástasis de rectos / eventración</option>
              <option value="general_pared_abdominal">General / detectar del informe</option>
            </select>
          </div>
          ...
          <div className="sm:col-span-2">
            <label className="block text-xs font-bold text-slate-700 mb-1 flex items-center gap-1">
              ...
              {scorecardData?.criteria?.length ? (
                <span className="inline-flex items-center gap-1 ml-1 px-1.5 py-0.5 rounded bg-amber-50 text-amber-800 border border-amber-200 text-[10px] font-black uppercase tracking-wide">
                  ...
                  Scorecard de pared obligatorio
                </span>
              ) : (
            ...
            <textarea
              ...
              placeholder="Ej: Hernia umbilical de 18 mm con grasa omental reducible — no inventar incarceración ni diástasis ausentes..."
            ...
            {scorecardData?.criteria?.length ? (
              <p className="mt-1 text-[10px] text-amber-800/90">
                Los criterios activos del Scorecard (y radar de pared / hernia si aplica) se inyectan como directiva obligatoria.
              </p>
            ) : null}
          </div>
        </div>
          ...
          <div className="text-xs text-slate-500">
            {abdominalWallData ? (
              <span className="text-emerald-700 font-semibold flex items-center gap-1">
                ...
                Suite Pared Abdominal 3D sincronizada ({abdominalWallData.panels?.length || 0} paneles generados)
              </span>
            ) : (
              "Presiona Generar para construir los modelos 3D y la tabla de pared abdominal"
            )}
          </div>
          ...
          >
            {isGenerating ? (
              <>
                ...
                <span>{generationStep || "Procesando Suite Pared Abdominal..."}</span>
              </>
            ) : (
              <>
                <Sparkles className="w-4 h-4 text-amber-100" />
                <span>{abdominalWallData ? "Re-generar Suite Pared Abdominal Completa" : "Generar Suite Pared Abdominal 3D con IA"}</span>
              </>
            )}
          </button>
        </div>
      ...
      {isGenerating && (
        <div className="p-12 text-center space-y-4">
          ...
          <div>
            <h4 className="font-bold text-slate-800 text-sm">{generationStep || "Generando reconstrucción 3D de pared abdominal..."}</h4>
            <p className="text-xs text-slate-500 max-w-md mx-auto mt-1">
              Renderizando capas de pared, orificio herniario, contenido y dinámica con Valsalva.
            </p>
          </div>
        </div>
      )}

      {!isGenerating && abdominalWallData && (
        <div className="p-5 space-y-6">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <span className="text-[11px] font-bold text-amber-900 uppercase tracking-wider block">
                  {abdominalWallData.territoryLabel || "ECOGRAFÍA DE PARED ABDOMINAL"} • {abdominalWallData.laterality || "—"}
                </span>
                <h4 className="text-xs font-bold text-slate-800">
                  {abdominalWallData.figureTitle || "FIGURA 1. ATLAS 3D PARED ABDOMINAL Y CORRELACIÓN DE DEFECTOS"}
                </h4>
              </div>
            </div>
            <button
          ...
          <div>
            <h4 className="text-xs font-bold text-slate-700 uppercase tracking-wider mb-3 flex items-center gap-1.5">
              ...
              Reconstrucción Volumétrica 3D Pared Abdominal ({abdominalWallData.panels?.length || 0} Paneles)
            </h4>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {(abdominalWallData.panels || []).map((panel, idx) => (
                <div
                  key={panel.id || idx}
                ...
                >
                  <div className="relative bg-slate-900 aspect-[4/3] group overflow-hidden flex items-center justify-center">
                    ...
                    <div className="absolute top-2 right-2 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity bg-black/60 backdrop-blur-sm p-1 rounded-lg">
                      ...
                      {abdominalWallData.panels.length > 1 && (
                        <button
                    ...
                    <div>
                      {isEditingText ? (
                        <input
                          ...
                          onChange={(e) => {
                            const updated = abdominalWallData.panels.map((p) =>
                              p.panelLetter === panel.panelLetter ? { ...p, panelTitle: e.target.value } : p
                            );
                            setAbdominalWallData({ ...abdominalWallData, panels: updated });
                          }}
                      ...
                      {editingPanelLetter === panel.panelLetter ? (
                        <div className="space-y-2 bg-slate-50 p-2 rounded border border-slate-200">
                          ...
                          <input
                            ...
                            placeholder="Ej: Orificio umbilical / Valsalva / diástasis — topografía exacta según informe..."
          ...
          <div className="rounded-2xl border border-amber-500/30 bg-gradient-to-br from-slate-950 via-slate-950 to-amber-950/40 p-4 space-y-5 text-slate-100">
            <p className="text-[10px] font-mono font-bold uppercase tracking-[0.18em] text-amber-300">Ficha clínica pared abdominal</p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="md:col-span-2 rounded-xl border border-amber-800/40 bg-slate-950/70 p-3">
                <p className="text-[10px] font-mono font-black uppercase tracking-widest text-amber-300 mb-2">Resumen de pared</p>
                {isEditingText ? (
                  <textarea rows={3} className="w-full text-xs bg-slate-900 border border-slate-700 rounded p-2 text-slate-100"
                    value={abdominalWallData.wallSummary || ""}
                    onChange={(e) => setAbdominalWallData({ ...abdominalWallData, wallSummary: e.target.value })} />
                ) : (
                  <p className="text-[13px] text-slate-200 leading-relaxed whitespace-pre-wrap">{abdominalWallData.wallSummary || "Sin resumen de pared."}</p>
                )}
              </div>
              <div className="min-w-0 rounded-xl border border-slate-700/70 bg-slate-950/70 p-3">
                <p className="text-[10px] font-mono font-black uppercase tracking-widest text-cyan-300 mb-2">Morfología del defecto</p>
                {isEditingText ? (
                  <textarea rows={5} className="w-full text-xs bg-slate-900 border border-slate-700 rounded p-2 text-slate-100"
                    value={abdominalWallData.morphologyNotes || ""}
                    onChange={(e) => setAbdominalWallData({ ...abdominalWallData, morphologyNotes: e.target.value })} />
                ) : (
                  <p className="text-[13px] text-slate-200 leading-relaxed whitespace-pre-wrap">{abdominalWallData.morphologyNotes || "Sin notas de morfología del defecto."}</p>
                )}
              </div>
              <div className="min-w-0 rounded-xl border border-slate-700/70 bg-slate-950/70 p-3">
                <p className="text-[10px] font-mono font-black uppercase tracking-widest text-yellow-300 mb-2">Capas / fascia</p>
                {isEditingText ? (
                  <textarea rows={5} className="w-full text-xs bg-slate-900 border border-slate-700 rounded p-2 text-slate-100"
                    value={abdominalWallData.wallLayersStatus || ""}
                    onChange={(e) => setAbdominalWallData({ ...abdominalWallData, wallLayersStatus: e.target.value })} />
                ) : (
                  <p className="text-[13px] text-slate-200 leading-relaxed whitespace-pre-wrap">{abdominalWallData.wallLayersStatus || "Sin descripción de capas/fascia."}</p>
                )}
              </div>
              <div className="md:col-span-2 rounded-xl border border-emerald-800/40 bg-slate-950/70 p-3">
                ...
                {isEditingText ? (
                  <textarea rows={4} className="w-full text-xs bg-slate-900 border border-slate-700 rounded p-2 text-slate-100"
                    value={(abdominalWallData.keyPoints || []).join("\n")}
                    onChange={(e) => setAbdominalWallData({ ...abdominalWallData, keyPoints: e.target.value.split("\n").map(s => s.trim()).filter(Boolean) })}
                    placeholder="Un punto por línea" />
                ) : (
                  <ul className="space-y-1.5">
                    {(abdominalWallData.keyPoints || []).length ? abdominalWallData.keyPoints!.map((kp, i) => (
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <h4 className="text-xs font-bold text-slate-700 uppercase tracking-wider flex items-center gap-1.5">
                ...
                {abdominalWallData.tableTitle || "TABLA ECOGRÁFICA DE PARED ABDOMINAL:"}
              </h4>
              <button
              ...
              <table className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="bg-slate-100 text-slate-700 font-bold border-b border-slate-200 text-[11px]">
                    <th className="py-2.5 px-3">{abdominalWallData.tableHeaders?.col1 || "LOCALIZACIÓN"}</th>
                    <th className="py-2.5 px-3">{abdominalWallData.tableHeaders?.col2 || "ESTRUCTURA"}</th>
                    <th className="py-2.5 px-3">{abdominalWallData.tableHeaders?.col3 || "DIÁMETRO / DIÁSTASIS"}</th>
                    <th className="py-2.5 px-3">{abdominalWallData.tableHeaders?.col4 || "CAPAS / FASCIA"}</th>
                    <th className="py-2.5 px-3">{abdominalWallData.tableHeaders?.col5 || "CONTENIDO"}</th>
                    <th className="py-2.5 px-3">{abdominalWallData.tableHeaders?.col6 || "REDUCIBILIDAD / VALSALVA"}</th>
                    <th className="py-2.5 px-3">{abdominalWallData.tableHeaders?.col7 || "SEVERIDAD"}</th>
                    <th className="py-2.5 px-3">{abdominalWallData.tableHeaders?.col8 || "IMPACTO"}</th>
                    <th className="py-2.5 px-2 w-8"></th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {(abdominalWallData.findingTable || []).map((row, idx) => (
                    <tr key={idx} className={idx % 2 === 1 ? "bg-slate-50/70" : "bg-white"}>
                      {([
                        ["location", row.location],
                        ["structure", row.structure],
                        ["sizeOrGap", row.sizeOrGap],
                        ["wallLayers", row.wallLayers],
                        ["content", row.content],
                        ["reducibilityOrDynamic", row.reducibilityOrDynamic],
                        ["severity", row.severity],
                        ["clinicalImpact", row.clinicalImpact],
                      ] as Array<[keyof AbdominalWallFindingRow, string]>).map(([field, val]) => (
          ...
          <div className="bg-cyan-50/70 border-l-4 border-amber-600 rounded-r-lg p-4 space-y-1.5">
            <h4 className="text-xs font-bold text-amber-800 uppercase tracking-wider">
              {abdominalWallData.synthesisTitle || "SÍNTESIS MORFOLÓGICA DE PARED ABDOMINAL:"}
            </h4>
            {isEditingText ? (
              <textarea
                rows={3}
                value={abdominalWallData.morphologicalSynthesis || ""}
                onChange={(e) =>
                  setAbdominalWallData({ ...abdominalWallData, morphologicalSynthesis: e.target.value })
                }
              ...
              <p className="text-xs text-slate-700 leading-relaxed">
                {abdominalWallData.morphologicalSynthesis}
              </p>
            )}
          </div>
        </div>
