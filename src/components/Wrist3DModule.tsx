...
import { buildWristDirectivesFromScorecard, WRIST_TOPOGRAPHY_DIRECTIVE } from "../lib/clinicalIntelligence";
...
export const Wrist3DModule: React.FC<Wrist3DModuleProps> = ({
  ...
  const [zoomPanel, setZoomPanel] = useState<Wrist3DPanel | null>(null);
    ...
    if (externalDirectives && externalDirectives.trim()) {
      setCustomDirectives((prev) => {
        if (
          ...
          prev.includes("DIRECTIVA OBLIGATORIA DEL SCORECARD DE MUÑECA")
        ) {
  ...
  React.useEffect(() => {
    const fromScorecard = buildWristDirectivesFromScorecard(scorecardData || null);
  ...
  const mergeMandatoryDirectives = (extraPanelDirective?: string) => {
    const scorecardDirectives = buildWristDirectivesFromScorecard(scorecardData || null);
    const userExtra = customDirectives.trim();
    const extraIsScorecardEcho =
      !!scorecardDirectives &&
      !!userExtra &&
      (userExtra === scorecardDirectives ||
        userExtra.includes("DIRECTIVA OBLIGATORIA DEL SCORECARD DE MUÑECA") ||
  ...
  const [selectedWristType, setSelectedWristType] = useState<WristStudyType>(() => {
    const text = ((reportText || "") + " " + (activeProtocol || "")).toLowerCase();
    if (/tfcc|triangulo fibrocartilaginoso|fibrocart[ií]lago triangular|ligamento escafolunar|escafolunar|lunotriquet|carpiano distal/.test(text)) return "tfcc_ligamentos";
    if (/t[uú]nel del carpo|carpal tunnel|mediano|flexor digitorum|flexores|retin[aá]culo flexor/.test(text)) return "tunel_carpiano";
    if (/tenosinovit|compartimento|extensor|de quervain|dequervain|ecu|extensor carpi|dorsal de mu[nñ]eca/.test(text)) return "tenosinovitis_dorsal";
    if (/mu[nñ]eca|wrist|carpo|carpal|radio-cubital|radiocarp/.test(text)) return "general_muneca";
    return "general_muneca";
  });
  ...
  const handleGenerate = async () => {
    if (!reportText || !reportText.trim()) {
      setErrorMessage("No hay informe disponible para procesar la suite de muñeca.");
      return;
    }

    setIsGenerating(true);
    setErrorMessage(null);
    setGenerationStep("Analizando tendones, túnel del carpo, TFCC y ligamentos de muñeca...");

    try {
      setGenerationStep("Construyendo paneles 3D y ficha de muñeca...");

      const mergedDirectives = mergeMandatoryDirectives();

      await runBackgroundTask("wrist-3d", "Generando Suite Muñeca 3D", async () => {
        const response = await fetch("/api/generate-3d-wrist", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            reportText,
            wristType: selectedWristType,
        ...
        if (!resData.success) {
          throw new Error(resData.error || "Error al generar la Suite Muñeca 3D.");
        }

        setWristData(resData.data);
        setIncludeInReport(true);
      });
      setGenerationStep("");
    } catch (err: any) {
      console.error("Error generando Suite Muñeca 3D:", err);
      setErrorMessage(err.message || "Error al procesar la Suite Muñeca.");
    } finally {
  ...
  const handleFlipHorizontal = async (panelLetter: string) => {
    if (!wristData) return;
    const panel = wristData.panels.find((p) => p.panelLetter === panelLetter);
    if (!panel?.imageUrl) return;
    try {
      const flippedDataUrl = await flipImageDataUrl(panel.imageUrl);
      const updatedPanels = wristData.panels.map((p) => {
      ...
      console.error("Error al voltear panel de muñeca:", flipErr);
  ...
  const handleDeleteSinglePanel = (panelLetter: string) => {
    if (!wristData || wristData.panels.length <= 1) return;

    const remainingPanels = wristData.panels.filter((p) => p.panelLetter !== panelLetter);
    ...
    let updatedTitle = wristData.figureTitle;
    if (updatedTitle) {
  ...
  const handleRegenerateSinglePanel = async (panel: Wrist3DPanel) => {
    if (!wristData) return;
    ...
    try {
      const response = await fetch("/api/regenerate-3d-wrist-panel", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          reportText,
      ...
      console.error("Error regenerando panel de muñeca:", err);
  ...
  const handleAddTableRow = () => {
    if (!wristData) return;
    const newRow: WristFindingRow = {
      location: "Muñeca derecha / 1er compartimento",
      structure: "Tendón / ligamento / compartimento",
      thicknessOrGap: "—",
      echoPattern: "Tenosinovitis / tendinopatía",
      hematomaOrFluid: "Sin líquido sinovial significativo",
      dynamicFinding: "Sin engatillamiento franco",
      severity: "Leve",
      clinicalImpact: "Seguimiento clínico"
    };
    setWristData({
      ...wristData,
      findingTable: [...(wristData.findingTable || []), newRow]
    });
  };

  const handleDeleteTableRow = (idx: number) => {
    if (!wristData) return;
    const updated = [...(wristData.findingTable || [])];
  ...
  const handleUpdateTableRow = (idx: number, field: keyof WristFindingRow, value: string) => {
    if (!wristData) return;
    const updated = [...(wristData.findingTable || [])];
    ...
    <div className="bg-white rounded-xl shadow-lg border border-sky-100 overflow-hidden transition-all duration-300">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-sky-100/80">
              Reconstrucción 3D de tendones dorsales/flexores, túnel del carpo, TFCC y ligamentos carpianos
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
          <div>
            <label className="block text-xs font-bold text-slate-700 mb-1 flex items-center gap-1">
              ...
              Tipo de estudio de muñeca
            </label>
            <select
              value={selectedWristType}
              onChange={(e) => setSelectedWristType(e.target.value as WristStudyType)}
            ...
            >
              <option value="tenosinovitis_dorsal">Tenosinovitis / tendones dorsales</option>
              <option value="tunel_carpiano">Túnel del carpo / flexores</option>
              <option value="tfcc_ligamentos">TFCC / ligamentos carpianos</option>
              <option value="general_muneca">General / detectar del informe</option>
            </select>
          </div>
          ...
          <div className="sm:col-span-2">
            <label className="block text-xs font-bold text-slate-700 mb-1 flex items-center gap-1">
              ...
              {scorecardData?.criteria?.length ? (
                <span className="inline-flex items-center gap-1 ml-1 px-1.5 py-0.5 rounded bg-sky-50 text-sky-800 border border-sky-200 text-[10px] font-black uppercase tracking-wide">
                  ...
                  Scorecard de muñeca obligatorio
                </span>
              ) : (
            ...
            <textarea
              ...
              placeholder="Ej: Tenosinovitis 1er compartimento derecho (De Quervain); TFCC ulnar intacto — no inventar rotura ausente..."
            ...
            {scorecardData?.criteria?.length ? (
              <p className="mt-1 text-[10px] text-sky-800/90">
                Los criterios activos del Scorecard (y radar MSK de muñeca si aplica) se inyectan como directiva obligatoria.
              </p>
            ) : null}
          </div>
        </div>
          ...
          <div className="text-xs text-slate-500">
            {wristData ? (
              <span className="text-emerald-700 font-semibold flex items-center gap-1">
                ...
                Suite Muñeca 3D sincronizada ({wristData.panels?.length || 0} paneles generados)
              </span>
            ) : (
              "Presiona Generar para construir los modelos 3D y la tabla de muñeca"
            )}
          </div>
          ...
          >
            {isGenerating ? (
              <>
                ...
                <span>{generationStep || "Procesando Suite Muñeca..."}</span>
              </>
            ) : (
              <>
                <Sparkles className="w-4 h-4 text-sky-100" />
                <span>{wristData ? "Re-generar Suite Muñeca Completa" : "Generar Suite Muñeca 3D con IA"}</span>
              </>
            )}
          </button>
        </div>
      ...
      {isGenerating && (
        <div className="p-12 text-center space-y-4">
          ...
          <div>
            <h4 className="font-bold text-slate-800 text-sm">{generationStep || "Generando reconstrucción 3D de muñeca..."}</h4>
            <p className="text-xs text-slate-500 max-w-md mx-auto mt-1">
              Renderizando tendones, retináculos, TFCC y topografía carpiana según el informe.
            </p>
          </div>
        </div>
      )}

      {!isGenerating && wristData && (
        <div className="p-5 space-y-6">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <span className="text-[11px] font-bold text-sky-900 uppercase tracking-wider block">
                  {wristData.territoryLabel || "ECOGRAFÍA MÚSCULO-TENDÓN"} • {wristData.laterality || "—"}
                </span>
                <h4 className="text-xs font-bold text-slate-800">
                  {wristData.figureTitle || "FIGURA 1. ATLAS 3D MÚSCULO-TENDÓN Y CORRELACIÓN ANATOMOPATOLÓGICA"}
                </h4>
              </div>
            </div>
            <button
          ...
          <div>
            <h4 className="text-xs font-bold text-slate-700 uppercase tracking-wider mb-3 flex items-center gap-1.5">
              ...
              Reconstrucción Volumétrica 3D Muñeca ({wristData.panels?.length || 0} Paneles)
            </h4>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {(wristData.panels || []).map((panel, idx) => (
                <div
                  key={panel.id || idx}
                ...
                >
                  <div className="relative bg-slate-900 aspect-[4/3] group overflow-hidden flex items-center justify-center">
                    ...
                    <div className="absolute top-2 right-2 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity bg-black/60 backdrop-blur-sm p-1 rounded-lg">
                      ...
                      {wristData.panels.length > 1 && (
                        <button
                          onClick={() => handleDeleteSinglePanel(panel.panelLetter)}
                          className="p-1 rounded text-white hover:bg-sky-500/40 transition-colors"
                    ...
                    <div>
                      {isEditingText ? (
                        <input
                          ...
                          onChange={(e) => {
                            const updated = wristData.panels.map((p) =>
                              p.panelLetter === panel.panelLetter ? { ...p, panelTitle: e.target.value } : p
                            );
                            setWristData({ ...wristData, panels: updated });
                          }}
          ...
          <div className="rounded-2xl border border-sky-500/30 bg-gradient-to-br from-slate-950 via-slate-950 to-sky-950/40 p-4 space-y-5 text-slate-100">
            <p className="text-[10px] font-mono font-bold uppercase tracking-[0.18em] text-sky-300">Ficha clínica de muñeca</p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="md:col-span-2 rounded-xl border border-sky-800/40 bg-slate-950/70 p-3">
                <p className="text-[10px] font-mono font-black uppercase tracking-widest text-sky-300 mb-2">Resumen muñeca</p>
                {isEditingText ? (
                  <textarea rows={3} className="w-full text-xs bg-slate-900 border border-slate-700 rounded p-2 text-slate-100"
                    value={wristData.wristSummary || ""}
                    onChange={(e) => setWristData({ ...wristData, wristSummary: e.target.value })} />
                ) : (
                  <p className="text-[13px] text-slate-200 leading-relaxed whitespace-pre-wrap">{wristData.wristSummary || "Sin resumen muñeca."}</p>
                )}
              </div>
              <div className="min-w-0 rounded-xl border border-slate-700/70 bg-slate-950/70 p-3">
                <p className="text-[10px] font-mono font-black uppercase tracking-widest text-cyan-300 mb-2">Morfología tendinosa / ligamentaria</p>
                {isEditingText ? (
                  <textarea rows={5} className="w-full text-xs bg-slate-900 border border-slate-700 rounded p-2 text-slate-100"
                    value={wristData.morphologyNotes || ""}
                    onChange={(e) => setWristData({ ...wristData, morphologyNotes: e.target.value })} />
                ) : (
                  <p className="text-[13px] text-slate-200 leading-relaxed whitespace-pre-wrap">{wristData.morphologyNotes || "Sin notas de morfología tendinosa/ligamentaria."}</p>
                )}
              </div>
              <div className="min-w-0 rounded-xl border border-slate-700/70 bg-slate-950/70 p-3">
                <p className="text-[10px] font-mono font-black uppercase tracking-widest text-yellow-300 mb-2">Tendones / ligamentos</p>
                {isEditingText ? (
                  <textarea rows={5} className="w-full text-xs bg-slate-900 border border-slate-700 rounded p-2 text-slate-100"
                    value={wristData.tendonLigamentStatus || ""}
                    onChange={(e) => setWristData({ ...wristData, tendonLigamentStatus: e.target.value })} />
                ) : (
                  <p className="text-[13px] text-slate-200 leading-relaxed whitespace-pre-wrap">{wristData.tendonLigamentStatus || "Sin descripción de tendones/ligamentos."}</p>
                )}
              </div>
              <div className="md:col-span-2 rounded-xl border border-emerald-800/40 bg-slate-950/70 p-3">
                ...
                {isEditingText ? (
                  <textarea rows={4} className="w-full text-xs bg-slate-900 border border-slate-700 rounded p-2 text-slate-100"
                    value={(wristData.keyPoints || []).join("\n")}
                    onChange={(e) => setWristData({ ...wristData, keyPoints: e.target.value.split("\n").map(s => s.trim()).filter(Boolean) })}
                    placeholder="Un punto por línea" />
                ) : (
                  <ul className="space-y-1.5">
                    {(wristData.keyPoints || []).length ? wristData.keyPoints!.map((kp, i) => (
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <h4 className="text-xs font-bold text-slate-700 uppercase tracking-wider flex items-center gap-1.5">
                ...
                {wristData.tableTitle || "TABLA ECOGRÁFICA DE MUÑECA:"}
              </h4>
              <button
              ...
              <table className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="bg-slate-100 text-slate-700 font-bold border-b border-slate-200 text-[11px]">
                    <th className="py-2.5 px-3">{wristData.tableHeaders?.col1 || "LADO / SITIO"}</th>
                    <th className="py-2.5 px-3">{wristData.tableHeaders?.col2 || "ESTRUCTURA"}</th>
                    <th className="py-2.5 px-3">{wristData.tableHeaders?.col3 || "GROSOR / ÁREA"}</th>
                    <th className="py-2.5 px-3">{wristData.tableHeaders?.col4 || "ECOPATRÓN"}</th>
                    <th className="py-2.5 px-3">{wristData.tableHeaders?.col5 || "LÍQUIDO / SINOVIAL"}</th>
                    <th className="py-2.5 px-3">{wristData.tableHeaders?.col6 || "DINÁMICA / ENGATILLAMIENTO"}</th>
                    <th className="py-2.5 px-3">{wristData.tableHeaders?.col7 || "SEVERIDAD"}</th>
                    <th className="py-2.5 px-3">{wristData.tableHeaders?.col8 || "IMPACTO"}</th>
                    <th className="py-2.5 px-2 w-8"></th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {(wristData.findingTable || []).map((row, idx) => (
                    <tr key={idx} className={idx % 2 === 1 ? "bg-slate-50/70" : "bg-white"}>
                      ...
                      ] as Array<[keyof WristFindingRow, string]>).map(([field, val]) => (
          ...
          <div className="bg-cyan-50/70 border-l-4 border-sky-600 rounded-r-lg p-4 space-y-1.5">
            <h4 className="text-xs font-bold text-sky-800 uppercase tracking-wider">
              {wristData.synthesisTitle || "SÍNTESIS MORFOLÓGICA DE MUÑECA:"}
            </h4>
            {isEditingText ? (
              <textarea
                rows={3}
                value={wristData.morphologicalSynthesis || ""}
                onChange={(e) =>
                  setWristData({ ...wristData, morphologicalSynthesis: e.target.value })
                }
              ...
              <p className="text-xs text-slate-700 leading-relaxed">
                {wristData.morphologicalSynthesis}
              </p>
            )}
          </div>
        </div>
