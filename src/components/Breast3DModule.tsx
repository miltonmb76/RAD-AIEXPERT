...
export const Breast3DModule: React.FC<Breast3DModuleProps> = ({
  ...
  const [zoomPanel, setZoomPanel] = useState<Breast3DPanel | null>(null);
    ...
    if (externalDirectives && externalDirectives.trim()) {
      setCustomDirectives((prev) => {
        if (
          ...
          prev.includes("DIRECTIVA OBLIGATORIA DEL SCORECARD MAMA")
        ) {
  ...
  const mergeMandatoryDirectives = (extraPanelDirective?: string) => {
    const scorecardDirectives = buildBreastDirectivesFromScorecard(scorecardData || null);
    ...
    const extraIsScorecardEcho =
      ...
      (userExtra === scorecardDirectives ||
        userExtra.includes("DIRECTIVA OBLIGATORIA DEL SCORECARD MAMA") ||
  ...
  const [selectedBreastType, setSelectedBreastType] = useState<BreastStudyType>(() => {
    const text = ((reportText || "") + " " + (activeProtocol || "")).toLowerCase();
    if (text.includes("axil") || text.includes("axila")) return "mama_axilas";
    if (text.includes("doppler") || text.includes("vascularidad")) return "mama_doppler";
    if (text.includes("mama") || text.includes("birads") || text.includes("bi-rads") || text.includes("breast")) return "mama_b_mode";
    return "general_breast";
  });
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      await runBackgroundTask("breast-3d", "Generando Suite Mama 3D", async () => {
        const response = await fetch("/api/generate-3d-breast", {
          method: "POST",
      ...
      console.error("Error generando Suite Mama 3D:", err);
      setErrorMessage(err.message || "Error al procesar la Suite Mama.");
    } finally {
  ...
  const handleDeleteSinglePanel = (panelLetter: string) => {
    if (!breastData || breastData.panels.length <= 1) return;

    const remainingPanels = breastData.panels.filter((p) => p.panelLetter !== panelLetter);
    ...
    let updatedTitle = breastData.figureTitle;
    if (updatedTitle) {
  ...
  const handleRegenerateSinglePanel = async (panel: Breast3DPanel) => {
    ...
    try {
      const response = await fetch("/api/regenerate-3d-breast-panel", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          reportText,
  ...
  const handleAddTableRow = () => {
    ...
    const newRow: BreastLesionRow = {
      ...
      orientation: "Paralelo",
      vascularity: "Ausente / periférico",
      biradsCategory: "BI-RADS 3",
      clinicalImpact: "Seguimiento"
    };
    setBreastData({
      ...breastData,
      lesionTable: [...breastData.lesionTable, newRow]
    });
  };
  ...
  const handleDeleteTableRow = (idx: number) => {
    if (!breastData) return;
    const updated = [...breastData.lesionTable];
  ...
  // Update row
  const handleUpdateTableRow = (idx: number, field: keyof BreastLesionRow, value: string) => {
    if (!breastData) return;
    const updated = [...breastData.lesionTable];
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg border border-pink-100 overflow-hidden transition-all duration-300">
      {/* Header Banner */}
      <div className="bg-gradient-to-r from-pink-900 via-pink-800 to-slate-900 px-5 py-4 text-white flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-lg bg-pink-700/60 border border-pink-400/30 flex items-center justify-center text-indigo-200">
            <Activity className="w-5 h-5 text-indigo-300" />
          </div>
          <div>
            <div className="flex items-center gap-2">
              <h3 className="font-bold text-base md:text-lg tracking-wide text-white">
                Suite Mama 3D & Ficha BI-RADS
              </h3>
              <span className="bg-pink-500/30 text-indigo-200 border border-pink-400/40 text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 rounded-full">
                Mamario Pro
              </span>
            </div>
            <p className="text-xs text-indigo-200/80">
              Reconstrucción macromama 3D de alta fidelidad, cálculo de estenosis y tabulación de velocidades
            </p>
          </div>
        </div>
      ...
      <div className="bg-slate-50 border-b border-slate-200 p-4 space-y-3">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => setSelectedBreastType(e.target.value as BreastStudyType)}
              className="w-full text-xs bg-white border border-slate-300 rounded-md px-2.5 py-1.5 text-slate-800 font-medium focus:ring-1 focus:ring-pink-500 focus:outline-none"
            >
              <option value="mama_b_mode">Mama B-mode / nódulos</option>
              <option value="mama_doppler">Mama + Doppler</option>
              <option value="mama_axilas">Mama + axilas</option>
              <option value="general_breast">General / detectar del informe</option>
            </select>
          </div>
        ...
        <div className="flex items-center justify-between pt-1">
          <div className="text-xs text-slate-500">
            {breastData ? (
              <span className="text-emerald-700 font-semibold flex items-center gap-1">
                ...
                Suite Mama 3D sincronizada ({breastData.panels?.length || 0} paneles generados)
              </span>
            ) : (
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-pink-600 to-pink-700 hover:from-pink-700 hover:to-pink-800 text-white rounded-lg text-xs font-bold shadow-md shadow-indigo-200 transition-all disabled:opacity-50"
          >
            {isGenerating ? (
              <>
                <Loader2 className="w-4 h-4 animate-spin text-white" />
                <span>{generationStep || "Procesando Suite Mama..."}</span>
              </>
            ) : (
              <>
                <Sparkles className="w-4 h-4 text-amber-300" />
                <span>{breastData ? "Re-generar Suite Mama Completa" : "Generar Suite Mama 3D con IA"}</span>
              </>
            )}
          </button>
        </div>
      ...
      {isGenerating && (
        <div className="p-12 text-center space-y-4">
          <div className="relative w-16 h-16 mx-auto">
            ...
            <Activity className="absolute inset-0 m-auto w-6 h-6 text-pink-600 animate-bounce" />
          </div>
          <div>
      ...
      {/* Main Content Area */}
      {!isGenerating && breastData && (
        <div className="p-5 space-y-6">
          {/* Section: Figure Title */}
          <div className="border border-pink-100 bg-pink-50/50 rounded-lg p-3 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                <span className="text-[11px] font-bold text-pink-900 uppercase tracking-wider block">
                  {breastData.territoryLabel || "DOPPLER VASCULAR"} • {breastData.laterality || "Bilateral"}
                </span>
                <h4 className="text-xs font-bold text-slate-800">
                  {breastData.figureTitle || "FIGURA 1. ATLAS 3D DE CORRELACIÓN ANATOMOPATOLÓGICA Y HEMODINÁMICA"}
                </h4>
              </div>
            </div>
            <button
                    ...
                    <div>
                      {isEditingText ? (
                        <input
                          ...
                          onChange={(e) => {
                            const updated = breastData.panels.map((p) =>
                              p.panelLetter === panel.panelLetter ? { ...p, panelTitle: e.target.value } : p
                            );
                            setBreastData({ ...breastData, panels: updated });
                          }}
                    ...
                    <div className="pt-2 border-t border-slate-100">
                      {editingPanelLetter === panel.panelLetter ? (
                        <div className="space-y-2 bg-slate-50 p-2 rounded border border-slate-200">
                          ...
                          <div className="flex items-center justify-end gap-1.5">
                            ...
                            <button
                              ...
                              className="px-2.5 py-1 text-[10px] bg-pink-600 hover:bg-pink-700 text-white font-bold rounded flex items-center gap-1"
                            >
                        ...
                        <button
                          ...
                          className="w-full py-1 text-[10px] text-pink-700 bg-pink-50 hover:bg-pink-100 font-semibold rounded flex items-center justify-center gap-1 transition-colors"
                        >
                          <RefreshCw className="w-2.5 h-2.5" />
              ...
              <div className="md:col-span-2 rounded-xl border border-pink-800/40 bg-slate-950/70 p-3">
                ...
                {isEditingText ? (
                  <textarea rows={3} className="w-full text-xs bg-slate-900 border border-slate-700 rounded p-2 text-slate-100"
                    ...
                    onChange={(e) => setBreastData({ ...breastData, breastSummary: e.target.value })} />
                ) : (
                  <p className="text-[13px] text-slate-200 leading-relaxed whitespace-pre-wrap">{breastData.breastSummary || "Sin resumen glandular."}</p>
                )}
              </div>
              <div className="min-w-0 rounded-xl border border-slate-700/70 bg-slate-950/70 p-3">
                ...
                {isEditingText ? (
                  <textarea rows={5} className="w-full text-xs bg-slate-900 border border-slate-700 rounded p-2 text-slate-100"
                    ...
                    onChange={(e) => setBreastData({ ...breastData, morphologyNotes: e.target.value })} />
                ) : (
                  <p className="text-[13px] text-slate-200 leading-relaxed whitespace-pre-wrap">{breastData.morphologyNotes || "Sin notas morfológicas."}</p>
                )}
              </div>
              <div className="min-w-0 rounded-xl border border-slate-700/70 bg-slate-950/70 p-3">
                ...
                {isEditingText ? (
                  <textarea rows={5} className="w-full text-xs bg-slate-900 border border-slate-700 rounded p-2 text-slate-100"
                    ...
                    onChange={(e) => setBreastData({ ...breastData, axillaryStatus: e.target.value })} />
                ) : (
                  <p className="text-[13px] text-slate-200 leading-relaxed whitespace-pre-wrap">{breastData.axillaryStatus || "Sin descripción ganglionar."}</p>
                )}
              </div>
              <div className="md:col-span-2 rounded-xl border border-emerald-800/40 bg-slate-950/70 p-3">
                ...
                {isEditingText ? (
                  <textarea rows={4} className="w-full text-xs bg-slate-900 border border-slate-700 rounded p-2 text-slate-100"
                    ...
                    onChange={(e) => setBreastData({ ...breastData, keyPoints: e.target.value.split("\n").map(s => s.trim()).filter(Boolean) })}
                    placeholder="Un punto por línea" />
                ) : (
                  <ul className="space-y-1.5">
                    {(breastData.keyPoints || []).length ? breastData.keyPoints!.map((kp, i) => (
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <h4 className="text-xs font-bold text-slate-700 uppercase tracking-wider flex items-center gap-1.5">
                ...
                {breastData.tableTitle || "TABLA BI-RADS Y CARACTERIZACIÓN DE LESIONES:"}
              </h4>
              <button
              ...
              <table className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="bg-slate-100 text-slate-700 font-bold border-b border-slate-200 text-[11px]">
                    ...
                    <th className="py-2.5 px-3">{breastData.tableHeaders?.col6 || "ORIENTACIÓN"}</th>
                    <th className="py-2.5 px-3">{breastData.tableHeaders?.col7 || "VASCULARIDAD"}</th>
                    <th className="py-2.5 px-3">{breastData.tableHeaders?.col8 || "BI-RADS"}</th>
                    <th className="py-2.5 px-3">{breastData.tableHeaders?.col9 || "IMPACTO"}</th>
                    <th className="py-2.5 px-2 w-8"></th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {breastData.lesionTable.map((row, idx) => (
                    <tr key={idx} className={idx % 2 === 1 ? "bg-slate-50/70" : "bg-white"}>
                      ...
                      <td className="py-2 px-3 text-fuchsia-700 font-semibold">
                        {isEditingText ? (
                          <input
                            type="text"
                            value={row.margins}
                      ...
                      <td className="py-2 px-3 text-slate-600">
                        {isEditingText ? (
                          <input type="text" value={row.orientation || ""} onChange={(e) => handleUpdateTableRow(idx, "orientation", e.target.value)} className="w-full text-xs border border-slate-300 rounded px-1.5 py-0.5" />
                        ) : (row.orientation || "—")}
                      </td>
                      <td className="py-2 px-3 text-slate-600">
                        {isEditingText ? (
                          <input
                            type="text"
                            value={row.vascularity || ""}
                            onChange={(e) => handleUpdateTableRow(idx, "vascularity", e.target.value)}
                            className="w-full text-xs border border-slate-300 rounded px-1.5 py-0.5"
                          />
                        ) : (
                          row.vascularity || "—"
                      ...
                      <td className="py-2 px-3 font-bold text-pink-800">
                        {isEditingText ? (
                          <input type="text" value={row.biradsCategory || ""} onChange={(e) => handleUpdateTableRow(idx, "biradsCategory", e.target.value)} className="w-full text-xs border border-slate-300 rounded px-1.5 py-0.5" />
                        ) : (row.biradsCategory || "—")}
                      </td>
                      <td className="py-2 px-3 text-slate-600">
                        {isEditingText ? (
          ...
          <div className="bg-rose-50/70 border-l-4 border-rose-600 rounded-r-lg p-4 space-y-1.5">
            <h4 className="text-xs font-bold text-rose-700 uppercase tracking-wider">
              {breastData.synthesisTitle || "SÍNTESIS MORFOLÓGICA Y BI-RADS:"}
            </h4>
            {isEditingText ? (
              <textarea
                rows={3}
                value={breastData.morphologicalSynthesis || ""}
                onChange={(e) =>
                  setBreastData({ ...breastData, morphologicalSynthesis: e.target.value })
                }
              ...
              <p className="text-xs text-slate-700 leading-relaxed">
                {breastData.morphologicalSynthesis}
              </p>
            )}
          </div>
        </div>
