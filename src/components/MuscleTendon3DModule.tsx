...
export const MuscleTendon3DModule: React.FC<MuscleTendon3DModuleProps> = ({
  ...
  const [zoomPanel, setZoomPanel] = useState<MuscleTendon3DPanel | null>(null);
    ...
    if (externalDirectives && externalDirectives.trim()) {
      setCustomDirectives((prev) => {
        if (
          ...
          prev.includes("DIRECTIVA OBLIGATORIA DEL SCORECARD MÚSCULO-TENDÓN")
        ) {
  ...
  React.useEffect(() => {
    const fromScorecard = buildMuscleTendonDirectivesFromScorecard(scorecardData || null);
  ...
  const mergeMandatoryDirectives = (extraPanelDirective?: string) => {
    const scorecardDirectives = buildMuscleTendonDirectivesFromScorecard(scorecardData || null);
    const userExtra = customDirectives.trim();
    const extraIsScorecardEcho =
      !!scorecardDirectives &&
      !!userExtra &&
      (userExtra === scorecardDirectives ||
        userExtra.includes("DIRECTIVA OBLIGATORIA DEL SCORECARD MÚSCULO-TENDÓN") ||
  ...
  const [selectedMuscleTendonType, setSelectedMuscleTendonType] = useState<MuscleTendonStudyType>(() => {
    const text = ((reportText || "") + " " + (activeProtocol || "")).toLowerCase();
    if (/aquiles|achilles|tend[oó]n de aquiles/.test(text)) return "aquiles_tendon";
    if (/miotendin|uni[oó]n miotendinosa|myotendinous|\bmtj\b/.test(text)) return "union_miotendinosa";
    if (/desgarro|rotura muscular|lesi[oó]n muscular|peetrons|isquiotibial|gemelo|gastrocnemio|s[oó]leo|cu[aá]driceps|rectus femoris|aductor/.test(text)) return "lesion_muscular";
    if (/m[uú]sculo|tend[oó]n|tendinos|miembro inferior|muslo|pantorrilla/.test(text)) return "general_musculo_tendon";
    return "general_musculo_tendon";
  });
  ...
  const handleGenerate = async () => {
    if (!reportText || !reportText.trim()) {
      setErrorMessage("No hay informe disponible para procesar la suite de músculo-tendón.");
      return;
    }

    setIsGenerating(true);
    setErrorMessage(null);
    setGenerationStep("Analizando lesión muscular/tendinosa, MTJ, Aquiles y dinámica...");

    try {
      setGenerationStep("Construyendo paneles 3D y ficha escrotal...");

      const mergedDirectives = mergeMandatoryDirectives();

      await runBackgroundTask("muscle-tendon-3d", "Generando Suite Muscular / Tendinosa 3D", async () => {
        const response = await fetch("/api/generate-3d-muscle-tendon", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            reportText,
            muscleTendonType: selectedMuscleTendonType,
      ...
      console.error("Error generando Suite Muscular / Tendinosa 3D:", err);
      setErrorMessage(err.message || "Error al procesar la Suite Muscular / Tendinosa.");
    } finally {
  ...
  const handleFlipHorizontal = async (panelLetter: string) => {
    ...
    const panel = muscleTendonData.panels.find((p) => p.panelLetter === panelLetter);
    if (!panel?.imageUrl) return;
    try {
      const flippedDataUrl = await flipImageDataUrl(panel.imageUrl);
      const updatedPanels = muscleTendonData.panels.map((p) => {
      ...
      console.error("Error al voltear panel de músculo-tendón:", flipErr);
  ...
  const handleDeleteSinglePanel = (panelLetter: string) => {
    if (!muscleTendonData || muscleTendonData.panels.length <= 1) return;

    const remainingPanels = muscleTendonData.panels.filter((p) => p.panelLetter !== panelLetter);
    ...
    let updatedTitle = muscleTendonData.figureTitle;
    if (updatedTitle) {
  ...
  const handleRegenerateSinglePanel = async (panel: MuscleTendon3DPanel) => {
    ...
    try {
      const response = await fetch("/api/regenerate-3d-muscle-tendon-panel", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          reportText,
      ...
      console.error("Error regenerando panel de músculo-tendón:", err);
  ...
  const handleAddTableRow = () => {
    if (!muscleTendonData) return;
    const newRow: MuscleTendonFindingRow = {
      location: "Pantorrilla derecha",
      structure: "Unión miotendinosa / vientre muscular",
      thicknessOrGap: "—",
      echoPattern: "Desgarro parcial / tendinopatía",
      hematomaOrFluid: "Sin colección significativa",
    ...
    setMuscleTendonData({
      ...muscleTendonData,
      findingTable: [...(muscleTendonData.findingTable || []), newRow]
    });
  };

  const handleDeleteTableRow = (idx: number) => {
    if (!muscleTendonData) return;
    const updated = [...(muscleTendonData.findingTable || [])];
  ...
  const handleUpdateTableRow = (idx: number, field: keyof MuscleTendonFindingRow, value: string) => {
    if (!muscleTendonData) return;
    const updated = [...(muscleTendonData.findingTable || [])];
    ...
    <div className="bg-white rounded-xl shadow-lg border border-amber-100 overflow-hidden transition-all duration-300">
      <div className="bg-gradient-to-r from-amber-900 via-orange-800 to-slate-900 px-5 py-4 text-white flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-lg bg-amber-700/60 border border-rose-400/30 flex items-center justify-center">
            <Activity className="w-5 h-5 text-amber-200" />
          </div>
          <div>
            <div className="flex items-center gap-2">
              <h3 className="font-bold text-base md:text-lg tracking-wide text-white">
                Suite Muscular / Tendinosa 3D & Ficha Músculo-Tendón
              </h3>
              <span className="bg-rose-500/30 text-amber-100 border border-rose-400/40 text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 rounded-full">
                Músculo-Tendón Pro
              </span>
            </div>
            <p className="text-xs text-amber-100/80">
              Reconstrucción 3D de desgarros musculares, uniones miotendinosas y tendones (p. ej. Aquiles)
            </p>
          </div>
        </div>
      ...
      <div className="bg-slate-50 border-b border-slate-200 p-4 space-y-3">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
          <div>
            <label className="block text-xs font-bold text-slate-700 mb-1 flex items-center gap-1">
              ...
              Tipo de estudio de músculo-tendón
            </label>
            <select
              value={selectedMuscleTendonType}
              onChange={(e) => setSelectedMuscleTendonType(e.target.value as MuscleTendonStudyType)}
              className="w-full text-xs bg-white border border-slate-300 rounded-md px-2.5 py-1.5 text-slate-800 font-medium focus:ring-1 focus:ring-rose-500 focus:outline-none"
            >
              <option value="lesion_muscular">Lesión muscular / desgarro</option>
              <option value="union_miotendinosa">Unión miotendinosa / MTJ</option>
              <option value="aquiles_tendon">Tendón de Aquiles</option>
              <option value="general_musculo_tendon">General / detectar del informe</option>
            </select>
          </div>
          ...
          <div className="sm:col-span-2">
            <label className="block text-xs font-bold text-slate-700 mb-1 flex items-center gap-1">
              ...
              {scorecardData?.criteria?.length ? (
                <span className="inline-flex items-center gap-1 ml-1 px-1.5 py-0.5 rounded bg-amber-50 text-rose-800 border border-amber-200 text-[10px] font-black uppercase tracking-wide">
                  <ShieldCheck className="w-3 h-3" />
                  Scorecard de músculo-tendón obligatorio
                </span>
              ) : (
            ...
            <textarea
              ...
              placeholder="Ej: Desgarro isquiotibial derecho en MTJ con gap 8 mm y hematoma; Aquiles midportion intacto — no inventar rotura completa ausente..."
              rows={scorecardData?.criteria?.length ? 5 : 2}
              className="w-full text-xs bg-white border border-slate-300 rounded-md px-2.5 py-1.5 text-slate-800 placeholder-slate-400 focus:ring-1 focus:ring-rose-500 focus:outline-none font-mono leading-relaxed"
            />
            {scorecardData?.criteria?.length ? (
              <p className="mt-1 text-[10px] text-rose-800/90">
                Los criterios activos del Scorecard (y radar de lesión muscular si aplica) se inyectan como directiva obligatoria.
              </p>
            ) : null}
          </div>
        </div>
        ...
        <div className="flex items-center justify-between pt-1">
          <div className="text-xs text-slate-500">
            {muscleTendonData ? (
              <span className="text-emerald-700 font-semibold flex items-center gap-1">
                ...
                Suite Muscular / Tendinosa 3D sincronizada ({muscleTendonData.panels?.length || 0} paneles generados)
              </span>
            ) : (
              "Presiona Generar para construir los modelos 3D y la tabla escrotal"
            )}
          </div>
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-rose-600 to-orange-700 hover:from-amber-700 hover:to-orange-800 text-white rounded-lg text-xs font-bold shadow-md shadow-amber-200 transition-all disabled:opacity-50"
          >
            {isGenerating ? (
              <>
                <Loader2 className="w-4 h-4 animate-spin text-white" />
                <span>{generationStep || "Procesando Suite Muscular / Tendinosa..."}</span>
              </>
            ) : (
              <>
                <Sparkles className="w-4 h-4 text-amber-100" />
                <span>{muscleTendonData ? "Re-generar Suite Muscular / Tendinosa Completa" : "Generar Suite Muscular / Tendinosa 3D con IA"}</span>
              </>
            )}
          </button>
        </div>
      ...
      {isGenerating && (
        <div className="p-12 text-center space-y-4">
          <div className="relative w-16 h-16 mx-auto">
            ...
            <Activity className="absolute inset-0 m-auto w-6 h-6 text-rose-600 animate-bounce" />
          </div>
          <div>
            <h4 className="font-bold text-slate-800 text-sm">{generationStep || "Generando reconstrucción 3D de músculo-tendón..."}</h4>
            <p className="text-xs text-slate-500 max-w-md mx-auto mt-1">
              Renderizando vientre muscular, MTJ, tendón y hematoma/retracción según el informe.
            </p>
          </div>
        </div>
      )}

      {!isGenerating && muscleTendonData && (
        <div className="p-5 space-y-6">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <span className="text-[11px] font-bold text-amber-900 uppercase tracking-wider block">
                  {muscleTendonData.territoryLabel || "ECOGRAFÍA MÚSCULO-TENDÓN"} • {muscleTendonData.laterality || "—"}
                </span>
                <h4 className="text-xs font-bold text-slate-800">
                  {muscleTendonData.figureTitle || "FIGURA 1. ATLAS 3D MÚSCULO-TENDÓN Y CORRELACIÓN ANATOMOPATOLÓGICA"}
                </h4>
              </div>
            </div>
            <button
                ...
                >
                  <div className="relative bg-slate-900 aspect-[4/3] group overflow-hidden flex items-center justify-center">
                    ...
                    <div className="absolute top-2 right-2 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity bg-black/60 backdrop-blur-sm p-1 rounded-lg">
                      ...
                      {muscleTendonData.panels.length > 1 && (
                        <button
                    ...
                    <div>
                      {isEditingText ? (
                        <input
                          ...
                          onChange={(e) => {
                            const updated = muscleTendonData.panels.map((p) =>
                              p.panelLetter === panel.panelLetter ? { ...p, panelTitle: e.target.value } : p
                            );
                            setMuscleTendonData({ ...muscleTendonData, panels: updated });
                          }}
                      ...
                      {editingPanelLetter === panel.panelLetter ? (
                        <div className="space-y-2 bg-slate-50 p-2 rounded border border-slate-200">
                          ...
                          <input
                            ...
                            placeholder="Ej: Isquiotibial derecho MTJ / Aquiles midportion / gap y retracción — topografía exacta según informe..."
                          ...
                          <div className="flex items-center justify-end gap-1.5">
                            ...
                            <button
                              ...
                              className="px-2.5 py-1 text-[10px] bg-rose-600 hover:bg-amber-700 text-white font-bold rounded flex items-center gap-1"
                            >
                        ...
                        <button
                          ...
                          className="w-full py-1 text-[10px] text-rose-800 bg-amber-50 hover:bg-amber-100 font-semibold rounded flex items-center justify-center gap-1 transition-colors"
                        >
                          <RefreshCw className="w-2.5 h-2.5" />
          ...
          <div className="rounded-2xl border border-rose-500/30 bg-gradient-to-br from-slate-950 via-slate-950 to-rose-950/40 p-4 space-y-5 text-slate-100">
            <p className="text-[10px] font-mono font-bold uppercase tracking-[0.18em] text-amber-300">Ficha clínica escrotal</p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="md:col-span-2 rounded-xl border border-rose-800/40 bg-slate-950/70 p-3">
                <p className="text-[10px] font-mono font-black uppercase tracking-widest text-amber-300 mb-2">Resumen músculo-tendón</p>
                {isEditingText ? (
                  <textarea rows={3} className="w-full text-xs bg-slate-900 border border-slate-700 rounded p-2 text-slate-100"
                    value={muscleTendonData.muscleTendonSummary || ""}
                    onChange={(e) => setMuscleTendonData({ ...muscleTendonData, muscleTendonSummary: e.target.value })} />
                ) : (
                  <p className="text-[13px] text-slate-200 leading-relaxed whitespace-pre-wrap">{muscleTendonData.muscleTendonSummary || "Sin resumen músculo-tendón."}</p>
                )}
              </div>
              <div className="min-w-0 rounded-xl border border-slate-700/70 bg-slate-950/70 p-3">
                <p className="text-[10px] font-mono font-black uppercase tracking-widest text-cyan-300 mb-2">Morfología testicular / epididimaria</p>
                {isEditingText ? (
                  <textarea rows={5} className="w-full text-xs bg-slate-900 border border-slate-700 rounded p-2 text-slate-100"
                    value={muscleTendonData.morphologyNotes || ""}
                    onChange={(e) => setMuscleTendonData({ ...muscleTendonData, morphologyNotes: e.target.value })} />
                ) : (
                  <p className="text-[13px] text-slate-200 leading-relaxed whitespace-pre-wrap">{muscleTendonData.morphologyNotes || "Sin notas de morfología muscular/tendinosa."}</p>
                )}
              </div>
              <div className="min-w-0 rounded-xl border border-slate-700/70 bg-slate-950/70 p-3">
                <p className="text-[10px] font-mono font-black uppercase tracking-widest text-yellow-300 mb-2">Doppler / vascular</p>
                {isEditingText ? (
                  <textarea rows={5} className="w-full text-xs bg-slate-900 border border-slate-700 rounded p-2 text-slate-100"
                    value={muscleTendonData.tearTendonStatus || ""}
                    onChange={(e) => setMuscleTendonData({ ...muscleTendonData, tearTendonStatus: e.target.value })} />
                ) : (
                  <p className="text-[13px] text-slate-200 leading-relaxed whitespace-pre-wrap">{muscleTendonData.tearTendonStatus || "Sin descripción de desgarro/tendón."}</p>
                )}
              </div>
              <div className="md:col-span-2 rounded-xl border border-emerald-800/40 bg-slate-950/70 p-3">
                <p className="text-[10px] font-mono font-black uppercase tracking-widest text-orange-300 mb-2">Puntos clave</p>
                {isEditingText ? (
                  <textarea rows={4} className="w-full text-xs bg-slate-900 border border-slate-700 rounded p-2 text-slate-100"
                    value={(muscleTendonData.keyPoints || []).join("\n")}
                    onChange={(e) => setMuscleTendonData({ ...muscleTendonData, keyPoints: e.target.value.split("\n").map(s => s.trim()).filter(Boolean) })}
                    placeholder="Un punto por línea" />
                ) : (
                  <ul className="space-y-1.5">
                    {(muscleTendonData.keyPoints || []).length ? muscleTendonData.keyPoints!.map((kp, i) => (
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <h4 className="text-xs font-bold text-slate-700 uppercase tracking-wider flex items-center gap-1.5">
                ...
                {muscleTendonData.tableTitle || "TABLA ECOGRÁFICA MÚSCULO-TENDÓN:"}
              </h4>
              <button
              ...
              <table className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="bg-slate-100 text-slate-700 font-bold border-b border-slate-200 text-[11px]">
                    <th className="py-2.5 px-3">{muscleTendonData.tableHeaders?.col1 || "LOCALIZACIÓN / LADO"}</th>
                    <th className="py-2.5 px-3">{muscleTendonData.tableHeaders?.col2 || "ESTRUCTURA"}</th>
                    <th className="py-2.5 px-3">{muscleTendonData.tableHeaders?.col3 || "GROSOR / GAP"}</th>
                    <th className="py-2.5 px-3">{muscleTendonData.tableHeaders?.col4 || "ECOPATRÓN"}</th>
                    <th className="py-2.5 px-3">{muscleTendonData.tableHeaders?.col5 || "HEMATOMA / LÍQUIDO"}</th>
                    <th className="py-2.5 px-3">{muscleTendonData.tableHeaders?.col6 || "DINÁMICA / RETRACCIÓN"}</th>
                    <th className="py-2.5 px-3">{muscleTendonData.tableHeaders?.col7 || "SEVERIDAD"}</th>
                    <th className="py-2.5 px-3">{muscleTendonData.tableHeaders?.col8 || "IMPACTO"}</th>
                    <th className="py-2.5 px-2 w-8"></th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {(muscleTendonData.findingTable || []).map((row, idx) => (
                    <tr key={idx} className={idx % 2 === 1 ? "bg-slate-50/70" : "bg-white"}>
                      {([
                        ...
                        ["hematomaOrFluid", row.hematomaOrFluid],
                        ["dynamicFinding", row.dynamicFinding],
                        ["severity", row.severity],
                        ["clinicalImpact", row.clinicalImpact],
                      ] as Array<[keyof MuscleTendonFindingRow, string]>).map(([field, val]) => (
          ...
          <div className="bg-cyan-50/70 border-l-4 border-rose-600 rounded-r-lg p-4 space-y-1.5">
            <h4 className="text-xs font-bold text-rose-800 uppercase tracking-wider">
              {muscleTendonData.synthesisTitle || "SÍNTESIS MORFOLÓGICA MÚSCULO-TENDINOSA:"}
            </h4>
            {isEditingText ? (
              <textarea
                rows={3}
                value={muscleTendonData.morphologicalSynthesis || ""}
                onChange={(e) =>
                  setMuscleTendonData({ ...muscleTendonData, morphologicalSynthesis: e.target.value })
                }
                className="w-full text-xs text-slate-800 border border-amber-300 rounded p-2 focus:ring-1 focus:ring-rose-500 focus:outline-none"
              />
            ) : (
              <p className="text-xs text-slate-700 leading-relaxed">
                {muscleTendonData.morphologicalSynthesis}
              </p>
            )}
          </div>
        </div>
