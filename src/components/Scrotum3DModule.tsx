...
import { buildScrotumDirectivesFromScorecard, SCROTUM_TOPOGRAPHY_DIRECTIVE } from "../lib/clinicalIntelligence";
...
export const Scrotum3DModule: React.FC<Scrotum3DModuleProps> = ({
  ...
  const [zoomPanel, setZoomPanel] = useState<Scrotum3DPanel | null>(null);
    ...
    if (externalDirectives && externalDirectives.trim()) {
      setCustomDirectives((prev) => {
        if (
          ...
          prev.includes("DIRECTIVA OBLIGATORIA DEL SCORECARD DE ESCROTO")
        ) {
  ...
  React.useEffect(() => {
    const fromScorecard = buildScrotumDirectivesFromScorecard(scorecardData || null);
  ...
  const mergeMandatoryDirectives = (extraPanelDirective?: string) => {
    const scorecardDirectives = buildScrotumDirectivesFromScorecard(scorecardData || null);
    const userExtra = customDirectives.trim();
    const extraIsScorecardEcho =
      !!scorecardDirectives &&
      !!userExtra &&
      (userExtra === scorecardDirectives ||
        userExtra.includes("DIRECTIVA OBLIGATORIA DEL SCORECARD DE ESCROTO") ||
  ...
  const [selectedScrotumType, setSelectedScrotumType] = useState<ScrotumStudyType>(() => {
    const text = ((reportText || "") + " " + (activeProtocol || "")).toLowerCase();
    if (text.includes("torsion") || text.includes("torsión") || text.includes("orquitis") || text.includes("epididimo-orquitis") || text.includes("epidídimo-orquitis") || text.includes("epididimoorquitis") || text.includes("agudo") || text.includes("aguda")) return "scrotum_acute";
    if (text.includes("varicocele") || text.includes("hidrocele") || text.includes("cronico") || text.includes("crónico") || text.includes("cronica") || text.includes("crónica")) return "scrotum_chronic";
    if (text.includes("masa") || text.includes("tumor") || text.includes("neoplasia") || text.includes("nodulo") || text.includes("nódulo") || text.includes("lesion focal") || text.includes("lesión focal")) return "scrotum_mass";
    if (text.includes("escroto") || text.includes("escrotal") || text.includes("testiculo") || text.includes("testículo") || text.includes("epididimo") || text.includes("epidídimo")) return "general_scrotum";
    return "general_scrotum";
  });
  ...
  const handleGenerate = async () => {
    if (!reportText || !reportText.trim()) {
      setErrorMessage("No hay informe disponible para procesar la suite de escroto.");
      return;
    }

    setIsGenerating(true);
    setErrorMessage(null);
    setGenerationStep("Analizando escroto, parénquima testicular, epidídimo y Doppler...");
    ...
    try {
      ...
      await runBackgroundTask("scrotum-3d", "Generando Suite Escroto 3D", async () => {
        const response = await fetch("/api/generate-3d-scrotum", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            reportText,
            scrotumType: selectedScrotumType,
        ...
        if (!resData.success) {
          throw new Error(resData.error || "Error al generar la Suite Escroto 3D.");
        }

        setScrotumData(resData.data);
        setIncludeInReport(true);
      });
      setGenerationStep("");
    } catch (err: any) {
      console.error("Error generando Suite Escroto 3D:", err);
      setErrorMessage(err.message || "Error al procesar la Suite Escroto.");
    } finally {
  ...
  const handleFlipHorizontal = async (panelLetter: string) => {
    if (!scrotumData) return;
    const panel = scrotumData.panels.find((p) => p.panelLetter === panelLetter);
    if (!panel?.imageUrl) return;
    try {
      const flippedDataUrl = await flipImageDataUrl(panel.imageUrl);
      const updatedPanels = scrotumData.panels.map((p) => {
      ...
      console.error("Error al voltear panel de escroto:", flipErr);
  ...
  const handleDeleteSinglePanel = (panelLetter: string) => {
    if (!scrotumData || scrotumData.panels.length <= 1) return;

    const remainingPanels = scrotumData.panels.filter((p) => p.panelLetter !== panelLetter);
    ...
    let updatedTitle = scrotumData.figureTitle;
    if (updatedTitle) {
  ...
  const handleRegenerateSinglePanel = async (panel: Scrotum3DPanel) => {
    if (!scrotumData) return;
    ...
    try {
      const response = await fetch("/api/regenerate-3d-scrotum-panel", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          reportText,
      ...
      console.error("Error regenerando panel de escroto:", err);
  ...
  const handleAddTableRow = () => {
    ...
    setScrotumData({
      ...scrotumData,
      findingTable: [...(scrotumData.findingTable || []), newRow]
    });
  };

  const handleDeleteTableRow = (idx: number) => {
    if (!scrotumData) return;
    const updated = [...(scrotumData.findingTable || [])];
  ...
  const handleUpdateTableRow = (idx: number, field: keyof ScrotumFindingRow, value: string) => {
    if (!scrotumData) return;
    const updated = [...(scrotumData.findingTable || [])];
    ...
    <div className="bg-white rounded-xl shadow-lg border border-amber-100 overflow-hidden transition-all duration-300">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="font-bold text-base md:text-lg tracking-wide text-white">
                Suite Escroto 3D & Ficha Escrotal
              </h3>
            ...
            <p className="text-xs text-amber-100/80">
              Reconstrucción 3D de testículos, epidídimo, cordón y Doppler escrotal
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
          <div>
            <label className="block text-xs font-bold text-slate-700 mb-1 flex items-center gap-1">
              ...
              Tipo de estudio de escroto
            </label>
            <select
              value={selectedScrotumType}
              onChange={(e) => setSelectedScrotumType(e.target.value as ScrotumStudyType)}
            ...
            >
              <option value="scrotum_acute">Agudo (torsión / epidídimo-orquitis)</option>
              <option value="scrotum_chronic">Crónico (varicocele / hidrocele)</option>
              <option value="scrotum_mass">Masa testicular / extratesticular</option>
              <option value="general_scrotum">General / detectar del informe</option>
            </select>
          </div>
          ...
          <div className="sm:col-span-2">
            <label className="block text-xs font-bold text-slate-700 mb-1 flex items-center gap-1">
              ...
              {scorecardData?.criteria?.length ? (
                <span className="inline-flex items-center gap-1 ml-1 px-1.5 py-0.5 rounded bg-amber-50 text-amber-800 border border-amber-200 text-[10px] font-black uppercase tracking-wide">
                  ...
                  Scorecard de escroto obligatorio
                </span>
              ) : (
            ...
            <textarea
              ...
              placeholder="Ej: Testículo derecho homogéneo 4,2×2,8×2,5 cm; Doppler hiliar presente — no inventar torsión ni masa ausentes..."
            ...
            {scorecardData?.criteria?.length ? (
              <p className="mt-1 text-[10px] text-amber-800/90">
                Los criterios activos del Scorecard (y radar escrotal si aplica) se inyectan como directiva obligatoria.
              </p>
            ) : null}
          </div>
        </div>
          ...
          <div className="text-xs text-slate-500">
            {scrotumData ? (
              <span className="text-emerald-700 font-semibold flex items-center gap-1">
                ...
                Suite Escroto 3D sincronizada ({scrotumData.panels?.length || 0} paneles generados)
              </span>
            ) : (
          ...
          >
            {isGenerating ? (
              <>
                ...
                <span>{generationStep || "Procesando Suite Escroto..."}</span>
              </>
            ) : (
              <>
                <Sparkles className="w-4 h-4 text-amber-100" />
                <span>{scrotumData ? "Re-generar Suite Escroto Completa" : "Generar Suite Escroto 3D con IA"}</span>
              </>
            )}
          </button>
        </div>
      ...
      {isGenerating && (
        <div className="p-12 text-center space-y-4">
          ...
          <div>
            <h4 className="font-bold text-slate-800 text-sm">{generationStep || "Generando reconstrucción 3D de escroto..."}</h4>
            <p className="text-xs text-slate-500 max-w-md mx-auto mt-1">
              Renderizando testículos, epidídimo, cordón y flujo Doppler según el informe.
            </p>
          </div>
        </div>
      )}

      {!isGenerating && scrotumData && (
        <div className="p-5 space-y-6">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <span className="text-[11px] font-bold text-amber-900 uppercase tracking-wider block">
                  {scrotumData.territoryLabel || "ECOGRAFÍA DE ESCROTO"} • {scrotumData.laterality || "—"}
                </span>
                <h4 className="text-xs font-bold text-slate-800">
                  {scrotumData.figureTitle || "FIGURA 1. ATLAS 3D ESCROTO Y CORRELACIÓN ANATOMOPATOLÓGICA"}
                </h4>
              </div>
            </div>
            <button
          ...
          <div>
            <h4 className="text-xs font-bold text-slate-700 uppercase tracking-wider mb-3 flex items-center gap-1.5">
              ...
              Reconstrucción Volumétrica 3D Escroto ({scrotumData.panels?.length || 0} Paneles)
            </h4>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {(scrotumData.panels || []).map((panel, idx) => (
                <div
                  key={panel.id || idx}
                    ...
                    <div>
                      {isEditingText ? (
                        <input
                          ...
                          onChange={(e) => {
                            const updated = scrotumData.panels.map((p) =>
                              p.panelLetter === panel.panelLetter ? { ...p, panelTitle: e.target.value } : p
                            );
                            setScrotumData({ ...scrotumData, panels: updated });
                          }}
                      ...
                      {editingPanelLetter === panel.panelLetter ? (
                        <div className="space-y-2 bg-slate-50 p-2 rounded border border-slate-200">
                          ...
                          <input
                            ...
                            placeholder="Ej: Testículo derecho / Doppler hiliar / varicocele izquierdo — topografía exacta según informe..."
          ...
          <div className="rounded-2xl border border-amber-500/30 bg-gradient-to-br from-slate-950 via-slate-950 to-amber-950/40 p-4 space-y-5 text-slate-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="md:col-span-2 rounded-xl border border-amber-800/40 bg-slate-950/70 p-3">
                <p className="text-[10px] font-mono font-black uppercase tracking-widest text-amber-300 mb-2">Resumen escrotal</p>
                {isEditingText ? (
                  <textarea rows={3} className="w-full text-xs bg-slate-900 border border-slate-700 rounded p-2 text-slate-100"
                    value={scrotumData.scrotumSummary || ""}
                    onChange={(e) => setScrotumData({ ...scrotumData, scrotumSummary: e.target.value })} />
                ) : (
                  <p className="text-[13px] text-slate-200 leading-relaxed whitespace-pre-wrap">{scrotumData.scrotumSummary || "Sin resumen escrotal."}</p>
                )}
              </div>
              <div className="min-w-0 rounded-xl border border-slate-700/70 bg-slate-950/70 p-3">
                ...
                {isEditingText ? (
                  <textarea rows={5} className="w-full text-xs bg-slate-900 border border-slate-700 rounded p-2 text-slate-100"
                    value={scrotumData.morphologyNotes || ""}
                    onChange={(e) => setScrotumData({ ...scrotumData, morphologyNotes: e.target.value })} />
                ) : (
                  <p className="text-[13px] text-slate-200 leading-relaxed whitespace-pre-wrap">{scrotumData.morphologyNotes || "Sin notas de morfología testicular/epididimaria."}</p>
                )}
              </div>
              <div className="min-w-0 rounded-xl border border-slate-700/70 bg-slate-950/70 p-3">
                ...
                {isEditingText ? (
                  <textarea rows={5} className="w-full text-xs bg-slate-900 border border-slate-700 rounded p-2 text-slate-100"
                    value={scrotumData.dopplerVascularStatus || ""}
                    onChange={(e) => setScrotumData({ ...scrotumData, dopplerVascularStatus: e.target.value })} />
                ) : (
                  <p className="text-[13px] text-slate-200 leading-relaxed whitespace-pre-wrap">{scrotumData.dopplerVascularStatus || "Sin descripción Doppler/vascular."}</p>
                )}
              </div>
              <div className="md:col-span-2 rounded-xl border border-emerald-800/40 bg-slate-950/70 p-3">
                ...
                {isEditingText ? (
                  <textarea rows={4} className="w-full text-xs bg-slate-900 border border-slate-700 rounded p-2 text-slate-100"
                    value={(scrotumData.keyPoints || []).join("\n")}
                    onChange={(e) => setScrotumData({ ...scrotumData, keyPoints: e.target.value.split("\n").map(s => s.trim()).filter(Boolean) })}
                    placeholder="Un punto por línea" />
                ) : (
                  <ul className="space-y-1.5">
                    {(scrotumData.keyPoints || []).length ? scrotumData.keyPoints!.map((kp, i) => (
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <h4 className="text-xs font-bold text-slate-700 uppercase tracking-wider flex items-center gap-1.5">
                ...
                {scrotumData.tableTitle || "TABLA ECOGRÁFICA DE ESCROTO:"}
              </h4>
              <button
              ...
              <table className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="bg-slate-100 text-slate-700 font-bold border-b border-slate-200 text-[11px]">
                    <th className="py-2.5 px-3">{scrotumData.tableHeaders?.col1 || "LADO / SITIO"}</th>
                    <th className="py-2.5 px-3">{scrotumData.tableHeaders?.col2 || "ESTRUCTURA"}</th>
                    <th className="py-2.5 px-3">{scrotumData.tableHeaders?.col3 || "TAMAÑO / VOLUMEN"}</th>
                    <th className="py-2.5 px-3">{scrotumData.tableHeaders?.col4 || "ECOPATRÓN"}</th>
                    <th className="py-2.5 px-3">{scrotumData.tableHeaders?.col5 || "DOPPLER / VASCULAR"}</th>
                    <th className="py-2.5 px-3">{scrotumData.tableHeaders?.col6 || "LÍQUIDO / MASA"}</th>
                    <th className="py-2.5 px-3">{scrotumData.tableHeaders?.col7 || "SEVERIDAD"}</th>
                    <th className="py-2.5 px-3">{scrotumData.tableHeaders?.col8 || "IMPACTO"}</th>
                    <th className="py-2.5 px-2 w-8"></th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {(scrotumData.findingTable || []).map((row, idx) => (
                    <tr key={idx} className={idx % 2 === 1 ? "bg-slate-50/70" : "bg-white"}>
                      {([
                        ["sideOrSite", row.sideOrSite],
                        ["structure", row.structure],
                        ["sizeOrVolume", row.sizeOrVolume],
                        ["echoPattern", row.echoPattern],
                        ["vascularOrDoppler", row.vascularOrDoppler],
                        ["fluidOrMass", row.fluidOrMass],
                        ["severity", row.severity],
                        ["clinicalImpact", row.clinicalImpact],
                      ] as Array<[keyof ScrotumFindingRow, string]>).map(([field, val]) => (
                        <td key={field} className={`py-2 px-3 ${field === "sideOrSite" || field === "structure" ? "font-semibold text-slate-900" : "text-slate-600"}`}>
                          {isEditingText ? (
                            <input
                              type="text"
                              value={val || ""}
          ...
          <div className="bg-cyan-50/70 border-l-4 border-amber-600 rounded-r-lg p-4 space-y-1.5">
            <h4 className="text-xs font-bold text-amber-800 uppercase tracking-wider">
              {scrotumData.synthesisTitle || "SÍNTESIS MORFOLÓGICA ESCROTAL:"}
            </h4>
            {isEditingText ? (
              <textarea
                rows={3}
                value={scrotumData.morphologicalSynthesis || ""}
                onChange={(e) =>
                  setScrotumData({ ...scrotumData, morphologicalSynthesis: e.target.value })
                }
              ...
              <p className="text-xs text-slate-700 leading-relaxed">
                {scrotumData.morphologicalSynthesis}
              </p>
            )}
          </div>
        </div>
